...
- **Tables** — frame → row → cell (frame) → content. Header row `surface-2` + bottom hairline, mono `muted` headers; body rows white with bottom hairlines; numeric columns right-justified; footer row `surface-2`.
